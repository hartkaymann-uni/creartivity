...
void main() {
	...
	//col = vec3( floor(mod( vPosition.z * u_thickness, 2)));
	//col = vec3( map( vPosition.z, -u_amplitude, u_amplitude, 0.0, 1.0 ) );
	//col = vec3( step(fract(vPosition.z), u_thickness));

	float f = fract(vPosition.z * u_thickness);
	float df = fwidth(f * u_thickness);
	float g = smoothstep(df * 1.0, df * 5.0, f);
	col = vec3(1 - g);

	//if(vPosition.z > u_amplitude * 0.66) col.yz = vec2(0.0);

	vFragColor = vec4( ( ambient + diffuse ) * col, 1.f);
	//vFragColor = vec4( vColor.xy, vColor.xy);
}